"""The utils here provide tensor transform for imgs."""
...
def auto_pad_images(image, grid_params):
    ...
    input_width, input_height = img_size
...
def select_best_resolution(original_size, possible_resolutions):
    ...
    """
    original_width, original_height = original_size[-2:]
    best_fit = None
...
def resize_and_pad_image(image, target_resolution):
    ...
    Args:
        image (PIL.Image): The input image, [c, w, h]
    ...
    if not _is_img_tensor:  # not Tensor then Image.Image
        # Resize the image
        ...
        resized_image = F.resize(image, [new_width, new_height])  # for img tensor input
        new_image = torch.zeros((3, target_width, target_height))
        new_image[:, paste_x:paste_x+new_width, paste_y:paste_y+new_height] = resized_image
...
def divide_to_patches(image, patch_size):
    ...
    Returns:
        list: A list of torch.Tensor image objects representing the patches.
    """
    patches = []
    if isinstance(image, Tensor):
        img_size = image.size()[-2:]
...
def process_anyres_image(image, processor, grid_pinpoints):
    ...
    if DEBUG_FIX_PADDING:
        ...
        image_original_resize = F.resize(image, [shortest_edge, shortest_edge])
...
def process_images(images: Tensor, image_processor, model_cfg):
